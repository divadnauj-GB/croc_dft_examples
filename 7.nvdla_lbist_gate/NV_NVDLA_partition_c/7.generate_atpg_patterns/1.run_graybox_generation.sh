...
read_design NV_NVDLA_partition_c  -design_id gate3 -verbose
set_current_design NV_NVDLA_partition_c

add_black_boxes -modules {\
                        RAMPDP_256X64_GL_M2_D2 \
                        RAMDP_16X64_GL_M1_E2 \
                        RAMDP_8X66_GL_M1_E2 \
...
foreach_in_collection mode_wrapper [get_config_elements Core(NV_NVDLA_partition_c)/Scan/Mode -part tcd -silent] {
  ...
  if {$mode_type eq "external"} {
    ...
    write_core_description ./NV_NVDLA_partition_c_gray_box.tcd -replace
  }
