...
foreach_in_collection mode_wrapper [get_config_elements Core(NV_NVDLA_partition_o)/Scan/Mode -part tcd -silent] {
  ...
  if {$mode_type eq "external"} {
    ...
    write_core_description ./NV_NVDLA_partition_o_gray_box.tcd -replace
  }
