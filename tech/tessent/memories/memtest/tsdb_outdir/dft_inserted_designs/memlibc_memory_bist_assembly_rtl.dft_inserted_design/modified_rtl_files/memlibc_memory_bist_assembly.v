...
module memlibc_memory_bist_assembly(BIST_CLK, m1_A_DLY, m1_A_DIN, m1_A_ADDR, m1_A_MEN, 
                                    m1_A_WEN, m1_A_REN, m1_A_DOUT, ijtag_tck, 
  ...
  memlibc_memory_bist_assembly_mem_container mem_container_inst(
      ...
      .m1_A_MEN(m1_A_MEN), .m1_A_WEN(m1_A_WEN), .m1_A_REN(m1_A_REN), .A_DOUT(m1_A_DOUT), 
                        .BIST_HOLD(BIST_HOLD), .BIST_SETUP(BIST_SETUP[0]), 
...
module memlibc_memory_bist_assembly_mem_container(BIST_CLK, m1_A_DLY, m1_A_DIN, 
                                                  ...
                                                  m1_A_REN, A_DOUT, BIST_HOLD, 
                        BIST_SETUP, BIST_SETUP_ts1, BIST_SETUP_ts2, 
  ...
  wire [12:0] A_ADDR;
  wire MEM0_BIST_COLLAR_SI, BIST_SO, BIST_GO, BIST_COLLAR_DIAG_EN, BIST_SELECT, 
       BIST_WRITEENABLE, BIST_READENABLE, BIST_CMP, BIST_COLLAR_EN0, 
       BIST_RUN_TO_COLLAR0, BIST_TESTDATA_SELECT_TO_COLLAR, BIST_ON_TO_COLLAR, 
  ...
  wire [31:0] A_DIN, A_DOUT_ts1;
  RM_IHPSG13_1P_8192x32_c4 m1_mem_inst(
      .A_CLK(BIST_CLK), .A_MEN(A_MEN), .A_WEN(A_WEN), .A_REN(A_REN), .A_ADDR(A_ADDR), 
      .A_DIN(A_DIN), .A_DLY(1'b1), .A_DOUT(A_DOUT_ts1)
  );
  ...
  memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller_inst(
      ...
      .CHKBCI_PHASE(CHKBCI_PHASE), .MBISTPG_GO(MBISTPG_GO), .MBISTPG_DONE(MBISTPG_DONE), 
  ...
  memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_interface_m1 m1_mem_inst_interface_inst(
      .A_MEN_IN(m1_A_MEN), .A_WEN_IN(m1_A_WEN), .A_REN_IN(m1_A_REN), .A_ADDR_IN(m1_A_ADDR[12:0]), 
      .A_DIN_IN(m1_A_DIN[31:0]), .A_DOUT_IN(A_DOUT_ts1), .TCK(to_interfaces_tck), 
      .BIST_CMP(BIST_CMP), .INCLUDE_MEM_RESULTS_REG(INCLUDE_MEM_RESULTS_REG), .BIST_SELECT(BIST_SELECT), 
      .BIST_WRITEENABLE(BIST_WRITEENABLE), .BIST_READENABLE(BIST_READENABLE), .BIST_COL_ADD(BIST_COL_ADD), 
      .BIST_ROW_ADD(BIST_ROW_ADD), .BIST_WRITE_DATA(BIST_WRITE_DATA), .CHKBCI_PHASE(CHKBCI_PHASE), 
      .BIST_TESTDATA_SELECT_TO_COLLAR(BIST_TESTDATA_SELECT_TO_COLLAR), .MEM_BYPASS_EN(memory_bypass_to_en), 
      .SCAN_SHIFT_EN(scan_to_en), .MCP_BOUNDING_EN(mcp_bounding_to_en), .BIST_ON(BIST_ON_TO_COLLAR), 
      .BIST_RUN(BIST_RUN_TO_COLLAR0), .BIST_ASYNC_RESETN(BIST_ASYNC_RESET), .BIST_CLK(BIST_CLK), 
      .BIST_SHIFT_COLLAR(BIST_SHIFT_COLLAR), .BIST_EXPECT_DATA(BIST_EXPECT_DATA), 
      ...
      .A_ADDR(A_ADDR), .A_DIN(A_DIN), .A_DOUT(A_DOUT[31:0]), .SCAN_OBS_FLOPS(), 
